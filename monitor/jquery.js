...
(function() {
    ...
    jQuery.extend({
        ...
        each: function(obj, fn, args) {
            if (args) {
                if (obj.length == undefined) for (var i in obj) fn.apply(obj[i], args);
                else for (var i = 0,
                ol = obj.length; i < ol; i++) if (fn.apply(obj[i], args) === false) break;
            } else {
                if (obj.length == undefined) for (var i in obj) fn.call(obj[i], i, obj[i]);
                else for (var i = 0,
                ol = obj.length,
                val = obj[0]; i < ol && fn.call(val, i, val) !== false; val = obj[++i]) {}
            }
            return obj;
        },
        ...
        className: {
            ...
            remove: function(elem, c) {
                elem.className = c != undefined ? jQuery.grep(elem.className.split(/\s+/),
                function(cur) {
        ...
        find: function(t, context) {
            ...
            while (t && last != t) {
                ...
                if (m) {
                    ...
                    for (var i = 0; ret[i]; i++) for (var c = ret[i].firstChild; c; c = c.nextSibling) if (c.nodeType == 1 && (nodeName == "*" || c.nodeName.toUpperCase() == nodeName.toUpperCase())) r.push(c);
                    ret = r;
                    t = t.replace(re, "");
            ...
            while (t && t != last) {
                ...
                if (m[1] == ":" && m[2] == "not") r = jQuery.filter(m[3], r, true).r;
                    ...
                    rl = r.length; i < rl; i++) {
                        ...
                        var add = false;
                        if (first == 1) {
                            if (last == 0 || node.nodeIndex == last) add = true;
    ...
    jQuery.event = {
        add: function(element, type, handler, data) {
            ...
            var handle = jQuery.data(element, "handle",
            function() {
                var val;
        ...
        remove: function(element, type, handler) {
            ...
            if (events) {
                ...
                if (!type) {
                    ...
                    if (!ret) {
                        if (element.removeEventListener) element.removeEventListener(type, jQuery.data(element, "handle"), false);
